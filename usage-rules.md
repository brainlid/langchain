...
- **Attach your own context with `custom_context[:otel_attributes]`** — a flat map of attribute name to value (string or atom keys). It lands on **every** span the chain produces (`invoke_agent`, each `chat`, each `execute_tool`), as does `gen_ai.conversation.id`. Only this reserved key is exported, never the rest of `custom_context`. Native types (string/number/boolean and homogeneous lists) are preserved; anything else is JSON-encoded; `nil` is dropped. Namespace your keys (`myapp.*`). Attributes a span derives itself always win over inherited ones, so you cannot mask `gen_ai.request.model`. Disable inheritance with `inherit_attributes: false`.
- **`LangChain.OpenTelemetry.Enrich`** is the supported way to enrich spans imperatively, and is **safe to call without OpenTelemetry installed** (both branches compile; no-ops when absent). `set_current_span_attributes/1` targets the innermost open span — the `execute_tool` span inside a tool function or tool callback, the chain span inside `:on_message_processed`. `put_inherited_attributes/1` seeds attributes before any chain exists (a Plug, LiveView mount, Oban worker) for every LangChain span opened later in that process. Do not call raw `OpenTelemetry.Tracer.set_attributes/1` against LangChain's spans; that couples you to internal bookkeeping.
- **`gen_ai.agent.name` / `gen_ai.agent.id`** come from `custom_context[:agent_name]` / `[:agent_id]`. Without `:agent_name`, `gen_ai.agent.name` falls back to the chain type (`"llm_chain"`), which is the same for every chain and groups nothing.
