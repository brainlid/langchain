defmodule LangChain.ChatModels.ChatGoogleAI do
  ...
  **Structured Output**
  ...
  The assistant message's content is a JSON string conforming to the schema;
  decode it with your JSON library (for example `Jason.decode!/1`).
  ...
  parameters are (see **Tool Schemas** above). Keywords with no field on
  Google's `Schema` type — `oneOf`, `allOf`, `const`, `$ref`, and others — are
  removed rather than rejected, so the request succeeds while those parts of
  the schema go unenforced. Schemas generated by tooling commonly use `$defs`
  and `$ref`; inline them first. See `LangChain.Utils.GoogleSchema` for the
  full list.
  """
  use Ecto.Schema
  require Logger
