defmodule LangChain.Agents.FileSystem.MultiPersistenceTest do
  ...
  describe "multiple persistence backends" do
    test "can register multiple persistence configs for different directories", %{
      ...
      assert FileSystemServer.get_persistence_configs(registry, agent_id) == %{}
      ...
      assert :ok = FileSystemServer.register_persistence(registry, agent_id, user_config)
      ...
      assert :ok = FileSystemServer.register_persistence(registry, agent_id, account_config)
      ...
      assert :ok = FileSystemServer.write_file(registry, agent_id, "/user_files/data.txt", "user data")
      ...
      assert {:error, reason} =
               FileSystemServer.write_file(registry, agent_id, "/account_files/data.txt", "account data")

      assert reason =~ "read-only"

      # Write to memory-only location
      assert :ok = FileSystemServer.write_file(registry, agent_id, "/scratch/temp.txt", "temp data")

      temp_entry = get_entry(agent_id, "/scratch/temp.txt")
      assert temp_entry.persistence == :memory
    end

    test "prevents registering same base_directory twice", %{
      agent_id: agent_id,
      tmp_dir: tmp_dir,
      registry: registry
    } do
      ...
      assert :ok = FileSystemServer.register_persistence(registry, agent_id, config1)
      ...
      assert {:error, reason} = FileSystemServer.register_persistence(registry, agent_id, config2)
    ...
    test "different configs can have different debounce times", %{
      ...
      assert :ok = FileSystemServer.register_persistence(registry, agent_id, fast_config)
      ...
      assert :ok = FileSystemServer.register_persistence(registry, agent_id, slow_config)

      # Write to both directories at the same time
      start_time = System.monotonic_time()
      FileSystemServer.write_file(registry, agent_id, "/fast/file.txt", "fast data")
      FileSystemServer.write_file(registry, agent_id, "/slow/file.txt", "slow data")
    ...
    test "readonly config prevents deletes", %{
      agent_id: agent_id,
      tmp_dir: tmp_dir,
      registry: registry
    } do
      ...
      assert :ok = FileSystemServer.register_persistence(registry, agent_id, config)

      # Try to delete from readonly directory (should fail)
      assert {:error, reason} = FileSystemServer.delete_file(registry, agent_id, "/readonly/file.txt")
      assert reason =~ "read-only"
    end
    ...
    test "lazy loads persisted file on first read", %{agent_id: agent_id, registry: registry} do
      ...
      assert {:ok, content} = FileSystemServer.read_file(registry, agent_id, "/data/existing.txt")
